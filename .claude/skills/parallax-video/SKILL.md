...
1. **`z0_prompt_typing`** (`mode: "follow"`): camera on the chat input box during prompt typing. `source_t: 0.0`, `duration` in SCRUBBED time = `(T_typing_end_raw / typing_factor) + ease_out` where `typing_factor` is from Phase 3 step 6 (Hard Rule #21 zone (a); usually `1.0` so duration = `T_typing_end_raw + ease`, but adaptive when raw typing >10s — e.g. 20s typing at `factor=2.0` gives scrubbed duration `10 + ease`). `region_pct: [40, 24, 38, 11]` (Cowork chrome default), `zoom: 1.5`, `ease: 1.5`.
...
2.  Record               → screen recordings/V<N>/vidN.mp4
                           Two modes:
                             ...
                             (b) Automated — "capture V<N>" triggers
                                 automation/capture.py (with scroll-through
                                 read-through dance — matches news visual
                                 language; Phase 5.5 freeze-frames during
                                 annotates). Manifest.json captures phases
                                 (Phase 3 reads streaming_started as T_typing_end).
                                 Trimmed.mp4 renamed → vidN.mp4 (canonical input);
                                 raw.mp4 renamed → vidN_raw.mp4 (archival).
