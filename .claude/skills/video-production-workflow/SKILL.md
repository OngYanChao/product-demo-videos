...
27. **The at-top frame must be held for ≥2.0s, must show the prompt + brief together at the top of the chat, and must flow naturally into the scroll-down without jump cuts.** When the automated screen capture trims `[streaming_ended → scroll_to_top_done]` (the synthetic scroll-up flicker), the viewer's first sight after the cut is the at-top frame — and that frame needs to show the *whole exchange* (prompt at the top of the chat + the brief below it), held long enough to register, and then transition smoothly into the read-through. With less than 2s, jump cuts, or a missing prompt, the cut into the scroll-down reads as jarring; with all three requirements satisfied, the eye lands on the prompt, traces down to the brief, and the scroll-down begins from a fully registered state with the same source playing on.

    **Three requirements, all load-bearing:**

    (a) **Hold duration ≥ 2.0s.** Calibrated against human reading-comprehension for a *two-element* visual (prompt + brief-title), not the single-element visual that 1s would suffice for. 2.0s gives the viewer time to read the prompt (the question they posed in the persona's voice), shift gaze to the brief beneath, and register "the response is here." Less than 2s feels cut-into; 2s feels deliberate; >3s starts feeling like dead time before the read-through. (Originally calibrated to 1.0s on 2026-05-30 morning; refined to 2.0s the same day after V3 visual review, when 1.0s with prompt-truncated framing was perceived as still rushed. The duration scaling tracks the visual complexity of the to-be-registered frame, not just the comprehension floor.)

    (b) **The frame must include the prompt at the top of the chat.** `automation/capture.py::scroll_chat_to_top()` must scroll the Cowork chat container all the way to the top — past the response, past any sticky brief header, to the FIRST message (the user's prompt). If the scroll stops at the top of the response and leaves the prompt off-screen above, the frame is incomplete for this rule's purpose. Implementation: 200 events × 10 lines per target = 2000 lines of scroll-up (raised from 80 events on 2026-05-30 — V3 visual review showed 80 events wasn't reaching the prompt for moderately long briefs; extra events are no-ops once the chat is actually at top, so over-providing is safe). **Cowork UI snap caveat:** after the scroll-up events stop firing, Cowork's UI runs a snap animation that can push the prompt off-screen by ~0.5-0.7s after `scroll_to_top_done`. The frame at `raw[scroll_to_top_done]` may therefore NOT show the prompt — the prompt was visible briefly DURING the scroll-up motion and got pushed off by the post-scroll snap. When this happens (visually verify the frame at `raw[scroll_to_top_done]`), find an earlier raw timestamp `t_at_top_prompt_visible` where the prompt is still visible (typically `scroll_to_top_done − 0.5 to 0.7s`). Use `t_at_top_prompt_visible` as the cut boundary instead of `streaming_ended` / `scroll_to_top_done` blindly. This issue is independent of scroll event count — it's about WHEN the prompt is visible during the scroll sequence, not whether the scroll reaches the top.

    (c) **Natural continuation from freeze into scroll-down — no jump cuts.** When the failsafe injects a freeze frame at the at-top position (to extend the hold duration), the source MUST continue from the same raw timestamp where the freeze slice began. The ffmpeg chain is `[source up to T_at_top] + [50ms slice at T_at_top + tpad clone padding] + [source from T_at_top onward]`. The post-freeze segment uses `trim=start=<T_at_top>` — NOT a later timestamp. NEVER skip source segments between the freeze and the post-freeze continuation. Skipping creates a visible jump cut where the frozen frame suddenly transitions to content from a visually-different later moment (e.g. freeze on prompt-visible frame → cut to mid-scroll-down). The freeze should "come alive" into the same content that immediately follows in the recording — typically the post-scroll snap animation (~0.5s of prompt scrolling off naturally), a brief settled state, and the smooth scroll-down read-through, all in natural recorded sequence. The viewer perceives: frozen prompt-visible frame → frame unfreezes and naturally transitions through the snap → scroll-down begins. Continuous, no synthetic jumps. Symptom of getting this wrong: viewer reports "after the freeze frame there's a sudden cut where it's already halfway scrolling down" (verbatim user observation that surfaced this sub-rule, 2026-05-30). Fix: change the v2 ffmpeg trim from `trim=start=<later_timestamp>` to `trim=start=<T_at_top>`.

    **Primary mechanism — capture-time hold.** `automation/capture.py` holds for `POST_SCROLL_TOP_HOLD_S = 2.0s` (since 2026-05-30) after scroll-to-top completes and before smooth-scroll-down begins. This bakes the 2s pause natively into every new recording. The `manifest.json`'s `smooth_scroll_chat_start − scroll_to_top_done` interval reflects this hold and should equal ~2.0s on captures made on/after 2026-05-30.

    **Failsafe — Phase 3 freeze-frame injection.** For recordings made before this default change (e.g. V3 captured 2026-05-29 with 0.5s baked in), or any future case where the manifest indicates `natural_top_hold < 2.0s`, the Phase 3 dispatch injects a freeze frame at the at-top position to extend the pause: `freeze_s = 2.0 - natural_top_hold`. The freeze frame is generated by `ffmpeg`'s `tpad=stop_mode=clone:stop_duration=<freeze_s>` filter cloning the last frame of a 50ms slice taken at the at-top position. Result: the final scrubbed file has ≥2.0s of held at-top frame before scroll-down begins, regardless of when the recording was captured. **Note:** the failsafe extends the pause but does NOT fix requirement (b) — if the recording didn't capture the prompt at the top of the frame, the freeze frame still won't include it. (b) requires the recording itself to have the prompt visible at the at-top moment, which the 2026-05-30 scroll-iteration bump addresses for new captures.
