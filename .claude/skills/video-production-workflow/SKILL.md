...
27. **The at-top frame must be held for at least 2.0s after the scroll-up cut, before smooth-scroll-down begins. The at-top frame must show the prompt AND the brief together at the top of the chat.** When the automated screen capture trims `[streaming_ended → scroll_to_top_done]` (the synthetic scroll-up flicker), the viewer's first sight after the cut is the at-top frame — and that frame needs to show the *whole exchange* (prompt at the top of the chat + the brief below it), not just the brief truncated at the top. The pause is editorial — viewer needs time to register "I asked X, and here's the response, ready to read" before the camera starts moving down. With less than 2s, the cut into the scroll-down reads as jarring; with 2s, the eye lands on the prompt, traces down to the brief, and the scroll-down begins from a fully registered state.
