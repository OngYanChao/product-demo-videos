...
| **26** | **Post-streaming pre-scroll-up dead-time cap (1s failsafe)** | **Yes** | Promoted from news's original N1 case (a). Lives in `automation/capture.py::cap_post_streaming_deadtime()`. Fires automatically when auto-trim can't run; no-op otherwise. |
...
|---|---|---|---|
| **N1** | **1s dead-time cap — case (b) only: trailing dead time after smooth-scroll-down completes, before the Polaris outro.** Catches the post-read-through tail dwell. **Note:** case (a) — the post-streaming pre-scroll-up dead time — was promoted to **global Hard Rule #26** on 2026-05-29; the failsafe now lives in `automation/capture.py` and applies to both pipelines (news + product-demo). N1's remaining scope is news-only (case (b)), since product-demo's post-scroll-down region is filled with Phase 5.5 annotate dwells (Hard Rule #25 panel-hold minimums) that must NOT be capped. Pre-tick (prompt typing) and tick-montage (phase-transition holds) are explicitly protected. | `tick_cut.cap_dead_times()` called from `process.py` after `zoom.py`; uses ffmpeg `freezedetect` (`-45dB`, `min_dead_s=1.0`). `protect_until_t` covers everything before z0b ease-out so case (a) is no-op when Hard Rule #26 already handled it. Disable per-run with `process.py --no-cap-dead`. | [`decisions/2026-05-28-news-1s-dead-time-cap.md`](../decisions/2026-05-28-news-1s-dead-time-cap.md) (original); [`decisions/2026-05-29-post-streaming-deadtime-cap-global.md`](../decisions/2026-05-29-post-streaming-deadtime-cap-global.md) (case (a) promotion) |
