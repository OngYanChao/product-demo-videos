...
| 2026-05-31 | **Hard Rule #29 — z0b ease-out plays over a frozen at-top frame + post-freeze raw-tail stitch.** Two requirements applied as a single rebuild step between scrub/tick-cut and `zoom.py`: (a) source content at z0b's ease-out window must be a freeze of the at-top frame (no live scroll/snap motion behind the camera retract); (b) after z0b ends + the 2s Rule #27 hold, source stitches `raw[T_at_top + 50ms, end]` for natural continuation into snap + smooth scroll-down + outro (no jump to compressed mid-scroll-down content). Auto-applied in Phase 3 dispatch when manifest + `scroll_to_top_done` + z0b are present. Extends Rule #27 (c)'s "no jump cuts" principle from the manual freeze-inject context to the auto-zoom workflow. N1 was the canary — Rule #27 fix alone left zoom-out reading as "scrolling while zooming back" + post-freeze jump cut to mid-scroll-down content; rebuild step satisfied both | [2026-05-31-zoom-out-over-still-and-raw-tail-stitch.md](2026-05-31-zoom-out-over-still-and-raw-tail-stitch.md) |
| 2026-06-01 | **Hard Rule #30 — static-placeholder gateway: skip z0b + 3s loading flash when no ticks detected.** Cowork's Progress sidebar has two UI modes during a response: (1) static placeholder ("See task progress for longer tasks." text + 3 dim circles, zero pixel motion); (2) dynamic checklist (numbered phases, blue indicator + checkmark accumulation). Detected by running `detect_ticks` probe over the loading window at the start of Phase 3 — 0 peaks = static, ≥1 = dynamic. Static path: skip z0b in zooms.json + ffmpeg pre-cut the loading window down to a fixed 3.0s flash (`T_typing_end + 0.2 + 3.0s` via 50ms slice + tpad clone). Dynamic path: standard z0b + tick-cut (news) / Variant B (product-demo) as today. Rules #27 + #29 don't fire on static path (no z0b to edit). Auto-applied in `news-pipeline/tools/process.py` + Phase 3 dispatch step 5. V3 (peer snapshot) was the canary — Cowork emitted the static placeholder; old pipeline zoomed in 2.5× on it for 17s of dead air | [2026-06-01-static-placeholder-gateway.md](2026-06-01-static-placeholder-gateway.md) |
| 2026-06-03 | **Hard Rule #22 amended (cross-pipeline), Hard Rule #31 new (z0→z0b zoom-out moment), News Rule N3 new (buffer-1 compression).** N2 capture exposed three gaps in news's auto-zoom path: (1) Rule #22 ("z0 ease-out at T_click") was implicit-product-demo-only; news's `tick_cut.py` emitted z0 covering the entire pre-tick segment (64.83s for N2 — viewer saw no zoom-out after the prompt fired). (2) No codified "zoom-out moment" between z0 and z0b — Rule #28's 2.5s buffer 1 minimum only ensured eases fit, didn't guarantee a held-still moment. (3) News's pre-first-tick wait (41s for N2) survived scrub + tick_cut. Codifications: Rule #22 explicitly cross-pipeline (news via `process.py::_rule_22_override_auto_zooms()` — rewrites `auto_zooms.json` after tick_cut). Rule #31 requires ≥1.5s full-frame view between z0 end and z0b start (extends Rule #28's buffer 1 minimum to 4.0s). Rule N3 compresses news's buffer-1 to 4.0s via `_compress_buffer_1()` (probes first-tick with magnitude ≥10 threshold, trims gap to 4s). N2 final: 103.4s → 64.08s with clear zoom-out moment | [2026-06-03-zoom-out-moment-and-buffer-1-compression.md](2026-06-03-zoom-out-moment-and-buffer-1-compression.md) |