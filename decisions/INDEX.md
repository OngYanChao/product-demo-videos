...
| 2026-05-30 | **Hard Rule #27 — minimum 1.0s top-hold pause before scroll-down.** The at-top brief frame (first visible after the scroll-up cut, when the brief sits at its top with the prompt visible above) must be held for ≥1.0s before smooth-scroll-down begins. Viewer needs the pause to register "brief is here, ready to read" before the camera starts moving. Two mechanisms: (1) primary — `automation/capture.py::POST_SCROLL_TOP_HOLD_S` bumped from 0.5 to 1.0 (bakes the pause natively into new captures); (2) failsafe — Phase 3 dispatch step 6b.5 injects a freeze frame via ffmpeg `tpad=stop_mode=clone` if `manifest.smooth_scroll_chat_start − scroll_to_top_done < 1.0s` (catches pre-2026-05-30 recordings like V3 with 0.5s baked in). Surfaced by V3 Variant B test — viewer reported the cut-into-mid-scroll as jarring. Scope: automated captures with readthrough scroll dance (both pipelines); no-op for `--no-readthrough` or manual recordings | [2026-05-30-minimum-top-hold.md](2026-05-30-minimum-top-hold.md) |