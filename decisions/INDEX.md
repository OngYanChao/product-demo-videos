...
| 2026-05-30 | **Hard Rule #27 — minimum 2.0s top-hold pause + prompt visible in at-top frame.** Two requirements: (a) hold duration ≥2.0s (raised from initial 1.0s proposal same day, after V3 visual review showed 1.0s was too rushed for the dual-element prompt+brief register the at-top frame needs to convey); (b) the at-top frame must include the prompt at the top of the chat — not just the response/brief title. Three mechanisms: (1) `automation/capture.py::scroll_chat_to_top()` raised from 80 to 200 scroll events per target (to actually reach the prompt above the response); (2) `POST_SCROLL_TOP_HOLD_S` bumped 0.5 → 2.0 (bakes 2s pause natively); (3) Phase 3 dispatch step 6b.5 freeze-frame failsafe via ffmpeg `tpad=stop_mode=clone` if `manifest.smooth_scroll_chat_start − scroll_to_top_done < 2.0s`. Failsafe fixes duration but NOT scroll position — pre-2026-05-30 recordings still need re-capture for full conformance. Surfaced by V3 Variant B test; two issues observed (scroll didn't reach prompt + 1.0s too rushed) addressed same day | [2026-05-30-minimum-top-hold.md](2026-05-30-minimum-top-hold.md) |